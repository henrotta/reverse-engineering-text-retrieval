...
const handleFileUploadAndResponse = (fileMethod) => (request, response) => {
  if (request.files.file) {
    ...
    const db_details = request.files.concepts
      ? JSON.parse(readFileSync(request.files.concepts[0].path, 'utf-8'))
      : undefined
    fileMethod
      .call(controller, zipTempFilePath, request.params.language, db_details)
      .then((result) => {
        response.status(200)
...
router.post(
  '/static/heuristics/language/:language/repository/zip',
  upload.single('file'),
  handleFileUploadAndResponse(controller.analyzeStaticallyHeuristics)
)

router.post(
  '/static/nlp/language/:language/repository/zip',
  upload.fields([{ name: 'file' }, { name: 'concepts', maxCount: 1 }]),
